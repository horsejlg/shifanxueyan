...
public class TopicController {
	...
	public int addVisibleUsersByClass(@PathVariable String topicId, @RequestBody ClassTeam classTeam) throws Exception {
		...
		return topicService.addVisibleUsersByClass(opUser, topicId, classTeam.getSpecialty(), classTeam.getGrade(),
				classTeam.getClasses());
	}
	...
	public int addParticipantsByClass(@PathVariable String topicId, @RequestBody ClassTeam classTeam) throws Exception {
		User opUser = AuthUtil.getCurrentUser();
		return topicService.addParticipantsByClass(opUser, topicId, classTeam.getSpecialty(), classTeam.getGrade(),
				classTeam.getClasses());
	}
	...
	public int deleteVisibleUsers(@PathVariable String topicId, @RequestBody Set<User> users,
			@PathVariable boolean isSynchronous) throws Exception {
		User opUser = AuthUtil.getCurrentUser();
	...
	public int deleteParticipants(@PathVariable String topicId, @RequestBody Set<User> users,
			@PathVariable boolean isSynchronous) throws Exception {
		User opUser = AuthUtil.getCurrentUser();
	...
	// 综合查询
	/**
	 * 出3个列表: 所有 (学生,老师) publish=1,visibleUsers=?(可不加,后面会自动加这个参数) 我参与的 (学生,老师)
	 * publish=1,participants=? 我发布的 (老师) author_id=?
	 * 
	 * @param params
	 * @param page
	 * @param pageSize
